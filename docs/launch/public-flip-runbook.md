...
diffs even though the working tree is scrubbed: the tailnet IP `100.103.0.55`
(~25 commits) and hostname `homelab.taildabf2.ts.net` (~24 commits). A public push
...
- **(A) Rewrite history with `git-filter-repo`** before the first public push.
  ...
  `git filter-repo --replace-text <(printf '%s\n' 'taildabf2==>example' '100.103.0.55==>100.64.0.55')`
