...
| `uv run hypermnesic doctor <home>/brain --check-dense-live --json` | Opt-in dense-live smoke confirms the embedding provider is reachable without printing secrets. | Doctor returned `status: ready`; dense live check passed. The production vault still reports stale/absent vectors, so broad convergence was not run in this issue. | Pass | Codex | 2026-06-14 | Yes |
