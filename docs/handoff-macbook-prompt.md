...
- Confirm the tailnet: `tailscale status` shows `homelab` (100.103.0.55).
...
PREREQ CHECK: `curl -s https://homelab.taildabf2.ts.net/cloud/.well-known/oauth-authorization-server`
...
- Guide the operator: in ChatGPT (Settings -> Connectors / "add MCP server") and in Claude
  ...
  https://homelab.taildabf2.ts.net/cloud/mcp
...
Verify an AUTHENTICATED round-trip (do not print the token):
  curl -s -H "Authorization: Bearer $HYPERMNESIC_MCP_TOKEN" -H 'Content-Type: application/json' \
       -H 'Accept: application/json' -X POST https://homelab.taildabf2.ts.net/mcp \
