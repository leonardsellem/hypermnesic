...
THE HOMELAB IS LIVE (deployed 2026-06-02): AS at http://100.64.0.55:8849 (tailnet),
auth-on write master at http://100.64.0.55:8851/mcp (tailnet, direct IP — NOT the hostname),
companion-read on http://100.64.0.55:8848 (read-only). The Mac identity (`mac`) is enrolled.
...
VERIFY an AUTHENTICATED round-trip (this is Gate-A criterion 5; do not print the token):
  curl -s -H "Authorization: Bearer $HYPERMNESIC_MCP_TOKEN" -H 'Content-Type: application/json' \
       -H 'Accept: application/json' -X POST http://100.64.0.55:8851/mcp \
