...
- Confirm the tailnet: `tailscale status` shows `homelab` (100.64.0.55).
...
PREREQ CHECK: `curl -s https://homelab.<tailnet-host>.ts.net/cloud/.well-known/oauth-authorization-server`
...
- Guide the operator: in ChatGPT (Settings -> Connectors / "add MCP server") and in Claude
  ...
  https://homelab.<tailnet-host>.ts.net/cloud/mcp
...
PREREQ CHECK: the hypermnesic tailnet endpoint must be live, OAuth-authenticated, and this Mac's
identity enrolled. The operator provides this Mac's token via env (e.g. HYPERMNESIC_MCP_TOKEN).
Verify an AUTHENTICATED round-trip (do not print the token):
  curl -s -H "Authorization: Bearer $HYPERMNESIC_MCP_TOKEN" -H 'Content-Type: application/json' \
       -H 'Accept: application/json' -X POST https://homelab.<tailnet-host>.ts.net/mcp \
       -d '{"jsonrpc":"2.0","id":1,"method":"tools/list","params":{}}'
must return the hypermnesic tool list (search/build_context/think/resolve [+ commit_note]); an
unauthenticated call must 401. If not live/enrolled, STOP and tell the operator which homelab
step (Gate-A rollout runbook) is missing.
