...
```text
You are a Claude Code session on a MacBook (a Tailscale tailnet peer; homelab = 100.103.0.55).
...
4) (OPTIONAL) ACTIVATE PROACTIVE AUTO-RECALL
   ...
   in your shell profile (the token is short-lived; mint it as in the Gate-A handoff):
     export HYPERMNESIC_MCP_URL=http://100.103.0.55:8851/mcp
...
and re-add it with the EXACT URL https://homelab.taildabf2.ts.net/cloud/mcp (NOT the bare /mcp —
