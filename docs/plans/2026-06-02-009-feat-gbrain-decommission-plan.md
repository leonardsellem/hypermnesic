...
(`homelab-hetzner-staging`, tailnet `100.64.0.55`) and the shared vault
`<home>/gbrain-brain` (*the vault*) — those targets are written as `~/…` or
...
- R12. hypermnesic's MCP is exposed as an **OAuth2-authenticated** endpoint (the engine as
  ...
  at the preserved hostname `homelab.<tailnet-host>.ts.net/mcp` via a **tailnet-internal
  `tailscale serve`** (not Funnel) pointed at hypermnesic on `100.64.0.55:8848`. The
...
- **gbrain serving:** `~/.config/systemd/user/gbrain-mcp-http.service` (`gbrain serve --http
  ...
  `homelab.<tailnet-host>.ts.net/mcp`. hypermnesic: `~/.config/systemd/user/hypermnesic.service`
  (`100.64.0.55:8848`, no auth, **not** behind the Funnel).
...
- **Preserve the hostname via tailnet-internal `tailscale serve`, not Funnel.** Keep
  `homelab.<tailnet-host>.ts.net/mcp` resolving to hypermnesic:8848, tailnet-only (no public
...
- [Affects R12][Needs research] **Can `honcho-oauth-proxy` mint audience-bound (RFC 8707) tokens
  for `resource_server_url = homelab.<tailnet-host>.ts.net/mcp` with the required scope?** If yes, U12
...
# CLI: serve --auth-issuer-url https://…/ --auth-resource-url https://homelab.<tailnet-host>.ts.net/mcp
...
- **Native-primitive first (non-negotiable): evaluate reusing `honcho-oauth-proxy`.** An OAuth 2.1
  ...
  and read/write scopes on the same `homelab.<tailnet-host>.ts.net` hostname (documented in
  `gbrain-brain/projects/homelab/services/honcho.md`). If it can mint **audience-bound** (RFC 8707)
  tokens for `resource_server_url = homelab.<tailnet-host>.ts.net/mcp` with the required scope, **U12
...
- **Resolve the dangling-target window:** the preserved hostname still resolves to gbrain until
  ...
  (`100.64.0.55:8848`), and U8 repoints both the hostname and the plugin's configured target to
...
> auth repoint of `homelab.<tailnet-host>.ts.net/mcp` is now owned by that plan: `/mcp` points at the
...
**Goal:** Repoint `homelab.<tailnet-host>.ts.net/mcp` from gbrain to the OAuth2-authenticated
...
- **Record the live serve map first** (`tailscale serve/funnel status`): today gbrain is at the
  ...
  change** — point `/mcp` (or the path consumers actually use) → hypermnesic `100.64.0.55:8848`
...
**Test scenarios:**
- Happy path: after the repoint, a call to `homelab.<tailnet-host>.ts.net/mcp` reaches hypermnesic
...
- **Interaction graph (what changes):** the preserved hostname `homelab.<tailnet-host>.ts.net/mcp`
