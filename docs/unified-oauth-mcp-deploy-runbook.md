...
| --- | --- | --- | --- |
| `/mcp` | `100.103.0.55:8851/mcp` | `hypermnesic-master-auth` (write master) | **repoint → unified; retire service** |
...
| (tailnet) | `100.103.0.55:8848` | `hypermnesic` (read master) | **retain (read companion)** |
| (tailnet) | `100.103.0.55:8849` | `hypermnesic-as` (client_credentials AS) | **retire (U7)** |
...
```
hypermnesic setup <repo> --public-url https://homelab.taildabf2.ts.net/mcp \
  --resource https://homelab.taildabf2.ts.net/mcp --port 8852   # distinct unit/port
...
curl -s -o /dev/null -w '%{http_code}\n' https://homelab.taildabf2.ts.net/.well-known/oauth-protected-resource/mcp   # 200
curl -s https://homelab.taildabf2.ts.net/.well-known/oauth-authorization-server/mcp | jq .token_endpoint            # non-null
...
curl -s -o /dev/null -w '%{http_code}\n' https://homelab.taildabf2.ts.net/honcho/.well-known/oauth-authorization-server  # 200
...
sudo tailscale funnel --bg --https=443 --set-path /mcp http://100.103.0.55:8851/mcp
