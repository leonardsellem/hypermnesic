...
**Captured:** 2026-06-02, on `homelab-hetzner-staging` (tailnet `100.103.0.55`)
...
| 8 | Obsidian related-notes panel populates over the tailnet | ▢ operator-side | Mac client config emitted at `~/.cache/hypermnesic-mac-client.json` (`mcpServers.hypermnesic` → `streamable-http` `http://100.103.0.55:8848/mcp`) — apply on the Mac + load Obsidian |
...
1. On the Mac: apply the emitted `mcpServers.hypermnesic` config, set the Obsidian
   plugin URL to `http://100.103.0.55:8848/mcp`, load Obsidian, confirm the
