...
- A2. gbrain — the outgoing layer: Supabase DB, the homelab MCP server (`https://homelab.taildabf2.ts.net/mcp`), the `gbrain` CLI, the restore cron; decommissioned at the end.
...
- **Assumption (tailnet reach):** the Mac and the homelab agents are both on the tailnet, so hypermnesic at `100.103.0.55:8848` is reachable from every consumer that uses gbrain today. If any consumer is off-tailnet, the deferred OAuth/remote reach becomes a prerequisite.
...
- [Affects R5][Needs research] Beyond the cron fleet, enumerate every gbrain consumer (the homelab remote MCP `homelab.taildabf2.ts.net/mcp`, manual `gbrain` CLI use, the Mac) so cutover is complete before teardown.
